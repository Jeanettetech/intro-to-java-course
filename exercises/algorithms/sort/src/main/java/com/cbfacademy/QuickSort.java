...
public class QuickSort implements ArraySorter {
	...
	// We are given the input array, the position of the starting element and the
	// position of the end element
	private int partition(int[] arr, int startIndex, int endIndex) {
		// Choose the rightmost element as the pivot
		int pivot = arr[endIndex];
		// Track the position of the element representing the boundary of the lower
		// partition and initialise it to the position just before the starting element
		int boundaryIndex = startIndex - 1;

		// Iterate through every element in the array except the pivot
		for (int currentIndex = startIndex; currentIndex < endIndex; currentIndex++) {
			// If the current element is less than the pivot, then increment the boundary
			// position and swap the element at the new boundary with the current element.
			// This has the effect of pushing all lower values to the left of the array.
	...
	// We are given the input array, the position of the left-hand element and the
	// position of the right-hand element
